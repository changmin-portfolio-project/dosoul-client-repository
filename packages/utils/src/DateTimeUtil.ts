import { NOT_DATE_TIME } from "@dosoul/consts";
...
export const formatDateTime = (inputDate: string): string => {
  ...
  return `${year}년 ${month}월 ${day}일. ${"0".repeat(2 - hour.toString().length)}${hour}시 ${"0".repeat(2 - minutes.toString().length)}${minutes}분`; // 원하는 형식으로 반환
};
