...
import { QueryStateRoomMoveoutDetail } from "@/hooks/query/QueryStateRoomMoveoutDetail";
import { postRoomMoveoutApprove } from "@/services/room/postRoomMoveoutAprove";
...
interface MoveOutApprovalPopupProps {
  isOpen: boolean;
  onClose: () => void;
  moveoutId: string;
  residentName: string;
...
export const MoveOutApprovalPopup: React.FC<MoveOutApprovalPopupProps> = ({
  isOpen,
  onClose,
  moveoutId,
  residentName,
  ...
  const { refetch: refetchRoomMoveoutList } = QueryStateAdminRoomMoveoutList();
  const { refetch: refetchMoveoutDetail } =
    QueryStateRoomMoveoutDetail(moveoutId);

  const handleApprovalComplete = () => {
    refetchRoomMoveoutList();
    refetchMoveoutDetail();
  };
  ...
  const handleApproval = async () => {
    ...
    // 거절인 경우 거절 사유 필수 입력
    if (selectedStatus === ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.value) {
    ...
    setIsLoading(true);
    try {
      await postRoomMoveoutApprove(
        moveoutId,
        selectedStatus,
        selectedStatus === ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.value
          ? rejectionReason
          : undefined,
      );
  ...
  const handleStatusChange = (status: string) => {
    ...
    // 상태가 변경되면 거절 사유 초기화
    if (status !== ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.value) {
      setRejectionReason("");
    }
  };
  ...
  const options = [
    {
      label: ROOM_MOVE_OUT_DOMAIN_STATUS.APPROVED.label,
      value: ROOM_MOVE_OUT_DOMAIN_STATUS.APPROVED.value,
      color: theme.mainColor.Blue,
    },
    {
      label: ROOM_MOVE_OUT_DOMAIN_STATUS.WAITING.label,
      value: ROOM_MOVE_OUT_DOMAIN_STATUS.WAITING.value,
      color: theme.grey.Grey6,
    },
    {
      label: ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.label,
      value: ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.value,
      color: theme.mainColor.Red,
    },
  ];

  return (
    <DynamicPopupTemplate title="퇴실 승인" onClose={onClose}>
      <PopupContainer>
        <PopupContent>
          <InfoSection>
            <InfoRow>
        ...
        <ButtonSection>
          ...
          <ConfirmButton
            ...
            disabled={
              ...
              !selectedStatus ||
              (selectedStatus ===
                ROOM_MOVE_OUT_DOMAIN_STATUS.DISAPPROVED.value &&
                isRejectionLength())
            }
          >
            {isLoading ? "처리 중..." : "확인"}
